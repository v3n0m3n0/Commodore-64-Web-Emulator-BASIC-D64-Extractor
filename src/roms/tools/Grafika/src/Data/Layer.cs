...
namespace PaintDotNet
{
          ...
          IDisposable
    {
              ...
              ISerializable
        {
            ...
            public bool isBackground;
            public byte opacity;
			public bool isFixClashes;
			public bool isIndexedColor;

            public const string IsBackgroundName = "Background Status";
            public const string NameName = "Name";
			public const string IsFixClashesName = "Fix Clashes Status";
			public const string IsIndexedColorName = "Indexed Color Status";
			public const string VisibleName = "Visibility";
            public const string OpacityName = "Opacity";

            public LayerProperties(string name, NameValueCollection userMetaData, bool isIndexedColor, bool isFixClashes, bool visible, bool isBackground, byte opacity)
            {
                this.name = name;
                this.userMetaData = new NameValueCollection(userMetaData);
				this.isFixClashes = isFixClashes;
				this.isIndexedColor = isIndexedColor;
            ...
            public LayerProperties(LayerProperties copyMe)
            {
				...
				this.isFixClashes = copyMe.isFixClashes;
				this.isIndexedColor = copyMe.isIndexedColor;
            ...
            public LayerProperties(SerializationInfo info, StreamingContext context)
            {
				...
				this.isFixClashes = info.GetBoolean("isFixClashes");
				this.isIndexedColor = info.GetBoolean("isIndexedColor");
            ...
            public void GetObjectData(SerializationInfo info, StreamingContext context)
            {
				...
				info.AddValue("isFixClashes", isFixClashes);
				info.AddValue("isIndexedColor", isIndexedColor);
		...
		public bool IsFixClashes
		{
			get
			{
				return properties.isFixClashes;
			}

			set
			{
				bool oldValue = properties.isFixClashes;

				if (oldValue != value)
				{
					OnPropertyChanging(LayerProperties.IsFixClashesName);
					properties.isFixClashes = value;
					OnPropertyChanged(LayerProperties.IsFixClashesName);
					Invalidate();
				}
			}
		}
		...
		public bool IsIndexedColor
		{
			get
			{
				return properties.isIndexedColor;
			}

			set
			{
				bool oldValue = properties.isIndexedColor;

				if (oldValue != value)
				{
					OnPropertyChanging(LayerProperties.IsIndexedColorName);
					properties.isIndexedColor = value;
					OnPropertyChanged(LayerProperties.IsIndexedColorName);
					Invalidate();
				}
			}
		}
        ...
        public virtual void LoadProperties(object oldState, bool suppressEvents)
        {
            ...
            if (!suppressEvents)
            {
				...
				if(lp.isIndexedColor != properties.isIndexedColor)
				{
					changed.Add(LayerProperties.IsIndexedColorName);
				}

				if(lp.isFixClashes != properties.isFixClashes)
				{
					changed.Add(LayerProperties.IsFixClashesName);
				}
        ...
        public void Invalidate(Rectangle roi)
        {
			...
			// HACK, we're doing the double pixel stuff
			if(this.properties.isIndexedColor)
			{
				rect.Width += rect.X%8;
				if(rect.Width%8 != 0) rect.Width += 8 - rect.Width%8;
				rect.X -= rect.X%8;
				rect.Height += rect.Y%8;
				if(rect.Height%8 != 0) rect.Height += 8 - rect.Height%8;
				rect.Y -= rect.Y%8;

				// compensate for weird stuff
				// rect.X += 1; rect.Y += 1;
				// rect.Width -= 2; rect.Height -= 2;
			}
			/*
			else
			if(this.properties.isDoublePixel)
			{
				rect.Width += rect.X%2 + rect.Width%2;
				rect.X -= rect.X%2;
			}
			*/
			// ENDHACK
        ...
        public Layer(int width, int height)
        {
            ...
            this.properties = new LayerProperties(null, new NameValueCollection(), false, false, true, false, 255);
        }
        ...
        public static BitmapLayer CreateBackgroundLayer(int width, int height)
        {
            ...
            BitmapLayer layer = new BitmapLayer(width, height, ColorBgra.Black); // HACK, was White

            layer.Name = "Layer 1";

            // tag it as a background layer
            layer.properties.isBackground = false; // true;
			// by default, it's single pixel hires
			// layer.properties.isDoublePixel = false;
			layer.properties.isIndexedColor = false;
