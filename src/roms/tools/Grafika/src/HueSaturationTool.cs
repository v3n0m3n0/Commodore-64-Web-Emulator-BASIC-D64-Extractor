...
{
	public class HueSaturationTool 
		: Tool
	{
		...
		*/
			...
			for (float f = 0; f < 1; f += fInc) 
			{
				...
				foreach (Rectangle rectSel in rectSelRegions)
				{
					...
					if (rectBrushArea.IntersectsWith(rectSel))
					{
						...
						float hue = ColorBgra.Hue(colorReplacing);
						float saturation = ColorBgra.Saturation(colorReplacing);
						...
						for (int y = rectBrushArea.Top; y < rectBrushArea.Bottom; y++)
						{
							...
							for (int x = rectBrushArea.Left; x < rectBrushArea.Right; x++)
							{
								if (srcBgra->A != 0)
								{
									...
									int hindex = (int)((hue+0.9f)*(31/6));
									int lindex = ColorBgra.LightIndex(ColorBgra.Index(colorLifted));

									if(( ((BitmapLayer)Workspace.ActiveLayer).IsIndexedColor))
									{
										if(saturation > 0.2)
										{
											colorAdjusted = ColorBgra.ConvertTable(hindex, lindex);
										}
										else
										{
											colorAdjusted = ColorBgra.ConvertGrayTable(lindex);
										}
									}
									else
									{
										colorAdjusted = ColorBgra.CalcHueSaturation(colorLifted, hue, saturation, true);
									}

									colorAdjusted.A = colorLifted.A;
		...
		public HueSaturationTool(DocumentWorkspace parent)
			: base(parent,
			Utility.GetImageResource("Icons.HueSaturationToolIcon.bmp"),
			"Hue/Saturation brush",
